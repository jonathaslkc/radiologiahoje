...
if(!isset($_SESSION)) { session_start(); }                                      // -------------------------------- INÍCIAR SESSÃO
	...
	if(isset($_FILES["file"]["type"])){
		...
		&& in_array($file_extension, $validextensions)) {
			...
			$extensao = pathinfo($_FILES['file']['name']);
			$extensao = ".".$extensao['extension'];
			$usuarioav= $_SESSION['usuario'];
			$_FILES['file']['name'] = $usuarioav.'avatar.'.'jpg';
			...
			if ($_FILES["file"]["error"] > 0){
				...
				//}else{
					...
					$targetPath = "images/sample/".$_FILES['file']['name']; // Target path where file is to be stored					
					...
					echo "<b>Tipo:</b> " . $_FILES["file"]["type"] . " - ";
					echo "<b>Tamanho:</b> " . ($_FILES["file"]["size"] / 1024) . " kB";
